...
<div class="w-full h-full p-4">
    <h1 class="mb-5 text-2xl font-semibold">Tambah Postingan Baru</h1>
    <form action="/posts" method="post" enctype="multipart/form-data">
        @csrf
        <div class="mb-4 w-auto">
            <input type="text" class="w-1/2 bg-white outline-none border-2 p-2 rounded-xl focus:border-blue-600 transition focus:ring-4 focus:ring-blue-200 {{ $errors->has('title') ? 'border-red-500' : 'border-slate-300' }}" placeholder="Title Postingan" name="title">
            @error('title')
        ...
        <div class="mb-4 w-auto">
            <div class="w-1/2 border-2 min-h-36 bg-white p-6 rounded-lg flex flex-col items-center justify-center cursor-pointer {{ $errors->has('image') ? 'border-red-500' : 'border-slate-300' }}" id="file-upload-wrapper">
                <img id="img-preview" class="hidden h-36 w-auto mb-4" alt="Image Preview"> 
