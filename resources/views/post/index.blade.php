...
<div class="p-5 bg-white rounded-lg w-full">
    <diV class="flex w-full justify-between items-center">
        <h1 class="text-lg font-semibold text-blue-600">Post list</h1>
        <a href="/posts/create" class="px-2 py-3 bg-blue-600 rounded-lg flex gap-2 text-white"><svg class="w-6 h-6 text-white dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                ...
                <li class="relative flex flex-row items-start w-full">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="w-1/2 flex items-center gap-3">
                            <a class="group inline-flex items-center h-9 rounded-lg text-sm font-semibold whitespace-nowrap px-4 py-3 focus:outline-none mt-6 bg-blue-600 text-white hover:bg-blue-500" href="/posts/{{ $item->id }}/edit">
                                Edit
                            </a>

                            <form action="/posts/{{ $item->id }}" method="post" class="p-0 m-0">
                                @csrf
                                @method('delete')
